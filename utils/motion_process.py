...
def recover_from_ric(data, pos_indices):
    # r_rot_quat, r_pos = recover_root_rot_pos(data)
    # positions = data[..., 4:(joints_num - 1) * 3 + 4]
    # positions = data[..., :joints_num * 3] #data[..., 3:3+joints_num * 3]

    positions = torch.cat((body_data, left_hand_data, right_hand_data), dim=1)
    # positions = positions.view(positions.shape[:-1] + (-1, 3))
    positions = positions.view(-1, joints_num, 3)
